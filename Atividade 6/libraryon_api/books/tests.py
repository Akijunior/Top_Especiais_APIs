...
class BooksTests(APITestCase):
    ...
    def test_create_and_retrieve_book(self):
        ...
        """
        new_book_title = 'New Game Category'
        response = self.create_book(new_book_title)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Book.objects.count(), 1)
        self.assertEqual(Book.objects.get().title,
                         new_book_title)
        print("PK {0}".format(Book.objects.get().pk))
    ...
    def test_create_duplicated_book(self):
        ...
        response1 = self.create_book(new_game_category_name)
        self.assertEqual(
            response1.status_code,
            status.HTTP_201_CREATED)
        response2 = self.create_book(new_game_category_name)
        self.assertEqual(
    ...
    def test_retrieve_game_categories_list(self):
        ...
        """
        new_game_category_name = 'New Game Category'
        self.create_book(new_game_category_name)
        url = reverse('gamecategory-list')
    ...
    def test_update_game_category(self):
        ...
        response = self.create_book(new_game_category_name)
        url = reverse(
            'gamecategory-detail',
            None,
            {response.data['pk']})
    ...
    def test_filter_game_category_by_name(self):
        ...
        self.create_book(game_category_name1)
        game_caregory_name2 = 'Second game category name'
        self.create_book(game_caregory_name2)
